...
export default class SetFace extends React.Component {
    ...
    async onCancel() {
        await AsyncStorage.setItem('facerecognitize', null)
        const {haveLocalAuth, sethaveLocalAuth} = this.context
        sethaveLocalAuth(false)
            ...
            <View style={styles.container}>
                ...
                <View style={[styles.panel, {height: height / 4}]}>
                    {this.state.setFinger ?
                        ...
                        (
                            <TouchableOpacity onPress={() => this.hasHardware()}>
                                <MaterialCommunityIcons name="face-recognition" size={100} color="#fff"/>
                            </TouchableOpacity>
                        )
                    }
                </View>
                <View style={styles.panel}>
                    <Text
                        style={styles.desc}>{t('loginregister.programlock.useFace')}</Text>
                </View>
                <View style={styles.panel}/>
            </View>
        )
